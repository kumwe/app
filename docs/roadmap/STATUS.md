...
| **Next** | Publish `kumwe/extension-sdk` `0.3.3` from its candidate (pull request 22, commit `b54022b0`), remove the temporary package repository entry from `composer.json`, re-point the lock at the Packagist distribution and re-verify the SDK attestation (`KUMWE-CS-2026-033`); adopt `kumwe/conversion-extension` once a successor release ships a record inside the StrictYaml subset (`KUMWE-MIG-2026-028`). The extension-sdk release train is adopted as `KUMWE-TRAIN-2026-033`: the SDK, contribution, automation, integration, record-query, reporting, interface-standard, administrator-contract, portal-contract and business-surface-contract entered through the migration ledger, the `kumwe/producer` `0.3.0 as 0.2.99` alias is retired and the legacy registry is empty. Computation Phase 2 then retires the App PHP executors. The programme of record is `docs/architecture/migrations/audits/2026-09-07/`; the ledger state is `docs/architecture/migrations/`. |
