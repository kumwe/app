...
| **Next** | Adopt the extension-sdk release train. `kumwe/extension-sdk` `0.3.2` selects sixteen exact Version 2 releases (access-control, administrator-contract, automation, business-policy, business-surface-contract, canonical-json, contribution, conversion `0.1.5`, idempotency, integration, portal-contract, producer `0.3.0`, record-model, record-query, record-values and reporting), and the maintainer ordered its adoption as the next App change on 2026-09-09 (`KUMWE-CS-2026-032`). It retires the `kumwe/producer` `0.3.0 as 0.2.99` alias and moves `kumwe/conversion` and `kumwe/extension-sdk` out of the legacy registry. The train lands package by package through the migration ledger in dependency order, each adoption removing the App duplicate the released record names; the remaining Content-and-Surfaces owners (business-definition, business-schema, content-model, audit, approval, interface-standard, conversion-extension) follow the same recipe, and Computation Phase 2 then retires the App PHP executors. The programme of record is `docs/architecture/migrations/audits/2026-09-07/`; the ledger state is `docs/architecture/migrations/`. |
...
| 12 | Studio contextual Content authoring ships and passes `STUDIO-PROD-015`, through PHP and with zero production Node.js/npm | Implemented, not yet qualified — the contextual shell mounts on Content New/Edit through Producer 0.3.0, whose adoption records are `core-integrated`, and the seven PHP authoring operations are proven end to end in integration; the packaged browser journey remains | `V2-STU-005` – `V2-STU-007` |
