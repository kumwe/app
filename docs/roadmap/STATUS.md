...
**42 open findings** in [`findings.json`](findings.json). The ledger holds open work only.
...
By severity: 0 critical, 17 high, 17 medium, 8 low.
By origin: 15 from the independent review, 12 still-open entries from the executed gap matrix, 15 discovered
...
journeys, 60 work items), `composer roadmap:check` (42 open findings), `composer openapi:check`, `composer
translation:check`, `composer translation:strings`, `composer assets:direction`, `composer cs`, `composer
analyse` (PHPStan level `max`, no errors), unit suite (1,845 tests, 25,655 assertions), architecture suite
...
canonical MariaDB leg produces, and the merge run is the first thing that will produce one. The suite
