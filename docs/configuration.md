...
| `EXTENSIONS_CONFORMANCE_ADMISSION` | How install-time admission treats the static code scan | `enforce` (default); `warn` records findings without refusing; `off` is refused in production |
