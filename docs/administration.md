...
Each installed release exposes a read-only **Declared contributions** diagnostic. It lists the manifest schema, package-owned capabilities, administrator workspaces/navigation, guarded routes, and templates/views, marking every declaration active or inactive. These records help operators review a package and diagnose lifecycle state; they do not edit provider-owned code. A capability declaration does not grant access. Assign it to the intended role under **Users and access** before its navigation or route becomes available.
