//#region ../app-release-pin/node_modules/@lit/reactive-element/css-tag.js
/**
* @license
...
//#endregion
//#region ../app-release-pin/node_modules/@lit/reactive-element/reactive-element.js
/**
* @license
...
//#endregion
//#region ../app-release-pin/node_modules/lit-html/lit-html.js
/**
* @license
* Copyright 2017 Google LLC
* SPDX-License-Identifier: BSD-3-Clause
...
//#endregion
//#region ../app-release-pin/node_modules/lit-element/lit-element.js
/**
* @license
* Copyright 2017 Google LLC
* SPDX-License-Identifier: BSD-3-Clause
...
//#endregion
//#region ../app-release-pin/node_modules/@lit/reactive-element/decorators/custom-element.js
/**
* @license
* Copyright 2017 Google LLC
* SPDX-License-Identifier: BSD-3-Clause
...
//#endregion
//#region ../app-release-pin/node_modules/@lit/reactive-element/decorators/property.js
/**
* @license
* Copyright 2017 Google LLC
* SPDX-License-Identifier: BSD-3-Clause
...
//#endregion
//#region ../app-release-pin/node_modules/@lit/reactive-element/decorators/state.js
/**
* @license
* Copyright 2017 Google LLC
* SPDX-License-Identifier: BSD-3-Clause
...
//#endregion
//#region assets/interface-standard/kis-tabs.ts
var KumweTabs = class KumweTabs extends i {
	pendingInvalid = null;
	invalidActivationTimer = null;
	...
	tabs() {
		return Array.from(this.querySelectorAll("[role=\"tab\"][data-kis-tab]"));
	}
	panels() {
		return Array.from(this.querySelectorAll("[role=\"tabpanel\"][data-kis-tab-panel]"));
	}
	selectedTab() {
		return this.querySelector("[role=\"tab\"][aria-selected=\"true\"]")?.dataset.kisTab ?? this.tabs()[0]?.dataset.kisTab ?? "";
	}
	locationTab() {
		const hashTarget = window.location.hash === "" ? null : document.getElementById(window.location.hash.slice(1));
		if (hashTarget instanceof HTMLElement && this.contains(hashTarget)) return hashTarget.closest("[data-kis-tab-panel]")?.dataset.kisTabPanel ?? null;
		const parameter = this.dataset.kisTabParameter ?? "tab";
		return new URL(window.location.href).searchParams.get(parameter);
	}
	activate(identifier, updateLocation, focus) {
		...
		for (const panel of this.panels()) panel.hidden = panel.dataset.kisTabPanel !== selected.dataset.kisTab;
		for (const input of this.querySelectorAll("input[type=\"hidden\"][name=\"return_tab\"]")) if (input.closest("kumwe-tabs") === this) input.value = selected.dataset.kisTab ?? "";
		if (updateLocation) {
	...
	handleClick = (event) => {
		...
		const tab = target.closest("[role=\"tab\"][data-kis-tab]");
	...
	handleKeydown = (event) => {
		const target = event.target;
		if (!(target instanceof HTMLAnchorElement) || target.getAttribute("role") !== "tab") return;
		const tabs = this.tabs();
	...
	handleInvalid = (event) => {
		...
		const identifier = target.closest("[data-kis-tab-panel]")?.dataset.kisTabPanel;
	...
	handleRevealTarget = (event) => {
		...
		const panel = event.target.closest("[data-kis-tab-panel]");
...
KumweTabs = __decorate([t("kumwe-tabs")], KumweTabs);
//#endregion
//#region assets/interface-standard/kis-master-detail.ts
var KumweMasterDetail = class KumweMasterDetail extends i {
	createRenderRoot() {
		return this;
	}
	connectedCallback() {
	...
	setOpen(open) {
		const toggle = this.querySelector("[data-kis-catalog-toggle]");
		this.toggleAttribute("data-catalog-open", open);
		toggle?.setAttribute("aria-expanded", String(open));
		if (open) this.querySelector("[data-kis-catalog] a, [data-kis-catalog] button")?.focus();
		else toggle?.focus();
	}
	handleClick = (event) => {
		const target = event.target;
		if (!(target instanceof Element)) return;
		if (target.closest("[data-kis-catalog-toggle]")) this.setOpen(!this.hasAttribute("data-catalog-open"));
		else if (target.closest("[data-kis-catalog] a")) this.setOpen(false);
	};
	handleKeydown = (event) => {
...
KumweMasterDetail = __decorate([t("kumwe-master-detail")], KumweMasterDetail);
//#endregion
//#region assets/interface-standard/kis-drawer.ts
var focusable = "a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex=\"-1\"])";
var KumweDrawer = class KumweDrawer extends i {
	...
	panel() {
		return this.querySelector("[data-kis-drawer-panel]");
	}
	open(control, focusFirst = true) {
		const panel = this.panel();
		if (!panel) return;
		...
		if (focusFirst) panel.querySelector(focusable)?.focus();
	}
	close(force = false) {
		const panel = this.panel();
		if (!panel) return;
		if (!force && panel.hasAttribute("data-dirty") && !window.confirm("Discard the unsaved drawer changes?")) return;
		panel.hidden = true;
		panel.removeAttribute("data-dirty");
		this.removeAttribute("data-open");
		this.querySelector("[data-kis-drawer-open]")?.setAttribute("aria-expanded", "false");
		document.body.classList.remove("kis-drawer-locked");
		this.invokingControl?.focus();
		this.invokingControl = null;
	}
	markDirty = (event) => {
		if (event.target instanceof Element && event.target.closest("[data-kis-drawer-panel]")) this.panel()?.setAttribute("data-dirty", "");
	};
	handleRevealTarget = (event) => {
		if (!(event.target instanceof Element) || !this.contains(event.target)) return;
		const control = this.querySelector("[data-kis-drawer-open]");
		if (control && !this.hasAttribute("data-open")) this.open(control, false);
	};
	handleClick = (event) => {
		const target = event.target;
		if (!(target instanceof Element)) return;
		const open = target.closest("[data-kis-drawer-open]");
	...
	handleKeydown = (event) => {
		...
		const controls = Array.from(panel.querySelectorAll(focusable));
...
KumweDrawer = __decorate([t("kumwe-drawer")], KumweDrawer);
//#endregion
//#region assets/interface-standard/copy-value.ts
function setupCopyValues(root = document) {
...
//#endregion
//#region assets/interface-standard/reveal-validation.ts
function setupValidationReveal(root = document) {
	root.addEventListener("click", (event) => {
		const source = event.target;
		if (!(source instanceof Element)) return;
		const link = source.closest(".kis-validation-summary a[href^=\"#\"]");
		if (!link) return;
		const identifier = decodeURIComponent(link.hash.slice(1));
		const target = root.getElementById(identifier);
		if (!(target instanceof HTMLElement)) return;
		event.preventDefault();
		target.dispatchEvent(new CustomEvent("kis:reveal-target", {
			bubbles: true,
			composed: true
		}));
