...
/**
 * The compiled registry of the pinned canonical Studio schemas Kumwe interprets.
 *
 * The exact schemas come from the pinned `@kumwe/studio-protocol` release vendored under
 * `resources/studio-contract/protocol/schemas/` behind the `composer studio:corpus` digest gate. The
 * same interpreter validates extension declarations and host-side content projections, so neither
 * bounded context grows a paraphrase of Studio's rules. The registry loads the complete reference
 * closure those documents need, checks every schema position against the closed interpreter grammar,
 * compiles the reviewed lexical patterns, and resolves local and cross-document references without
 * network access.
 *
 * @since  2.0.0
 */
final class StudioContractSchemas
{
    /**
    ...
    /**
     * Documents the App validates at runtime, including declarations and model-port projections.
     *
     * @var    list<string>
     * @since  2.0.0
     */
    ...
    /**
     * The reference closure: every runtime document schema and the documents it may reach.
     *
     * @var    list<string>
     * @since  2.0.0
     */
    ...
    /**
     * Legacy contribution-only closure used by the Extension compatibility adapter.
     *
     * @var    list<string>
     * @since  2.0.0
     */
    private const array CONTRIBUTION_CLOSURE = [
        ...self::CONTRIBUTION_KINDS,
        'blueprint',
        'common',
    ];
    ...
    /**
     * One compiled validator per runtime document kind, created on first use.
     *
    ...
    /**
     * Document roots by runtime document kind.
     *
    ...
    /**
     * Compile only the original contribution closure for the Extension compatibility adapter.
     *
     * @param   ?string  $schemaDirectory  Corpus schema directory; the vendored default when null.
     *
     * @return  self  Registry exposing the six contribution validators only.
     *
     * @since   2.0.0
     */
    public static function fromContributionCorpus(?string $schemaDirectory = null): self
    {
        /** @var self|null $shared */
        static $shared = null;
        if ($schemaDirectory === null) {
            return $shared ??= self::compileFrom(self::vendoredDirectory(), self::CONTRIBUTION_CLOSURE);
        }

        return self::compileFrom($schemaDirectory, self::CONTRIBUTION_CLOSURE);
    }
    ...
    {
        return self::compileFrom(self::vendoredDirectory());
    }

    /**
     * Resolve the exact repository-owned protocol schema directory after the namespace extraction.
     *
     * @return  string  Absolute vendored schema directory.
     *
     * @since   2.0.0
     */
    private static function vendoredDirectory(): string
    {
     ...
     * @param   list<string>  $closure          Exact schema-document closure to compile.
     *
     * @return  self  The compiled registry.
     *
     * @since   2.0.0
     */
    private static function compileFrom(
        string $schemaDirectory,
        array $closure = self::DOCUMENT_CLOSURE,
    ): self {
        $documents = [];
    ...
    /**
     * Hand out the interpreting validator for one runtime document kind.
     *
     ...
     *
     * @throws  LogicException  When the kind is not a supported canonical document kind.
     *
     * @since   2.0.0
     */
    public function validator(string $kind): SchemaPropertyValidator
    {
        ...
        if ($root === null) {
            throw new LogicException(sprintf('"%s" is not a supported canonical Studio document kind.', $kind));
        }
