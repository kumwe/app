...
/**
 * Pins that the business.record.idempotency.purge job drains its store inside the catalogue budget.
 *
...
final class PurgeBusinessRecordIdempotencyHandlerTest extends TestCase
{
    ...
    public function testAnEmptyPayloadDrainsWithTheDeclaredBudget(): void
    {
        ...
        self::assertSame(RetentionStore::BusinessIdempotency, $drain->requests[0]['store']);
        self::assertEquals(
            RetentionCatalogue::declared()->policy(RetentionStore::BusinessIdempotency)->budget(),
    ...
    public function testABatchSizeBeyondTheDeclaredBoundIsRefused(): void
    {
        $drain = new RecordingRetentionDrain();
        try {
            $this->handler($drain)->handle(['batch_size' => 1_001], $this->context());
    ...
    {
        self::assertSame('business.record.idempotency.purge', $this->handler(new RecordingRetentionDrain())->type());
    }
     ...
     *
     * @return  PurgeBusinessRecordIdempotencyHandler  Handler.
     *
     * @since   2.0.0
     */
    private function handler(RecordingRetentionDrain $drain): PurgeBusinessRecordIdempotencyHandler
    {
