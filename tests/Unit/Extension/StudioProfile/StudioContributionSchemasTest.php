...
final class StudioContributionSchemasTest extends TestCase
{
    /**
    ...
    /**
     * The vendored corpus compiles into one registry serving a validator per contribution kind.
     *
     * @return  void
     *
     * @since   2.0.0
     */
    public function testTheVendoredCorpusCompilesAValidatorForEveryContributionKind(): void
    {
        $registry = StudioContributionSchemas::fromVendoredCorpus(self::corpusDirectory());

        foreach (StudioContributionSchemas::CONTRIBUTION_KINDS as $kind) {
    ...
    {
        self::assertSame(
            StudioContributionSchemas::fromVendoredCorpus(),
            StudioContributionSchemas::fromVendoredCorpus(),
        );
    }

    /**
     * Asking for a validator outside the closed contribution-kind set is a programming error.
     *
     * @return  void
     *
     * @since   2.0.0
     */
    public function testAValidatorOutsideTheContributionKindsIsRefused(): void
    {
        $registry = StudioContributionSchemas::fromVendoredCorpus(self::corpusDirectory());

        $this->expectException(LogicException::class);
        $this->expectExceptionMessage('"blueprint" is not a canonical Studio contribution kind.');
        $registry->validator('blueprint');
    }
    ...
    public function testAPoisonedCorpusIsRefusedAtTheArmItBreaks(): void
    {
        ...
        foreach ($cases as $label => [$mutations, $message]) {
            ...
            $refused = null;
            try {
                StudioContributionSchemas::fromVendoredCorpus($directory);
    ...
    public function testPatternCompilationTranslatesOnlyRealUnicodeEscapes(): void
    {
        ...
        $registry = StudioContributionSchemas::fromVendoredCorpus($directory);
