...
<!-- List each record this PR regenerated or edited; a gate checks every one of them, so none needs a tick. -->
...
- `CHANGELOG.md`
- `docs/roadmap/acceptance-record.json`, with `docs/roadmap/STATUS.md` and `docs/roadmap/acceptance-summary.md` regenerated by `composer acceptance:summary`; `docs/roadmap/findings.json`
...
Automated workflow evidence and the maintainer's merge, once every required check is green, are the sole
acceptance record (ADR 0021). No human checkbox, manual browser, Safari, right-to-left or screen-reader review,
or follow-up acceptance commit is required; the merged documentation must already say what the merge accepts,
and anything still open stays open in `docs/roadmap/acceptance-record.json`.
